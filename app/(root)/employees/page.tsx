  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/employees/search?name=${encodeURIComponent(
          name
        )}&administration=${encodeURIComponent(administration)}`
      );
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center text-slate-600 font-medium text-xl animate-pulse">
          جاري تحميل البيانات...
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-none px-6 lg:px-12 xl:px-16 py-12 space-y-8">
        ...
        <div className="text-center space-y-6 mb-12">
          ...
          <h1 className="text-5xl md:text-6xl font-semibold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent leading-tight tracking-tight">
            إدارة الموظفين
          </h1>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-slate-200">
          ...
          <div className="p-8">
            <EmployeesTable
              employees={isSearching ? searchResults : employees}
            />
          </div>
        </div>
        ...
        <div className="text-center space-y-4 pt-8">
          <div className="inline-flex items-center justify-center space-x-2 space-x-reverse text-slate-600"></div>
